...
class hyperdb extends wpdb {
	...
	/**
	 * Whether to save db_debug_backtrace in save_query_callback. You may wish
		...
		if ( preg_match('/^\s*SELECT\s+FOUND_ROWS(\s*)/i', $query) && is_resource($this->last_found_rows_result) ) {
			...
			if ( $this->save_queries ) {
				if ( is_callable($this->save_query_callback) )
					$this->queries[] = call_user_func_array( $this->save_query_callback, array( $query, $elapsed, $this->save_backtrace ? db_debug_backtrace( false ) : null, &$this ) );
				else
	...
	function get_caller() {
		// requires PHP 4.3+
		if ( !is_callable('db_debug_backtrace') )
			return '';

		$bt = db_debug_backtrace( false );
		$caller = '';
